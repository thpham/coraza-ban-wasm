...
// syncScoreToRedis syncs a score entry to Redis
func (ctx *httpContext) syncScoreToRedis(entry *ScoreEntry) {
	if ctx.config.RedisCluster == "" {
		return
	}
	...
	entryJSON, err := entry.ToJSON()
	if err != nil {
		ctx.logError("failed to serialize score entry: %v", err)
		return
	}
	...
	_, err = proxywasm.DispatchHttpCall(
		...
		redisTimeout,
		func(numHeaders, bodySize, numTrailers int) {
			ctx.logDebug("score synced to Redis for %s", entry.Fingerprint)
		},
	)

	if err != nil {
		ctx.logError("failed to dispatch Redis score sync: %v", err)
	}
}

// getRedisScore retrieves a score from Redis
func (ctx *httpContext) getRedisScoreAsync(fingerprint string, callback func(*ScoreEntry, bool)) {
	if ctx.config.RedisCluster == "" {
		callback(nil, false)
		return
	}
	...
	_, err := proxywasm.DispatchHttpCall(
		...
		func(numHeaders, bodySize, numTrailers int) {
			...
			entry, found := ctx.parseRedisScoreResponse(body)
			callback(entry, found)
		},
	)

	if err != nil {
		ctx.logError("failed to dispatch Redis score get: %v", err)
		callback(nil, false)
	}
}

// parseRedisScoreResponse parses the Redis GET response for score
func (ctx *httpContext) parseRedisScoreResponse(body []byte) (*ScoreEntry, bool) {
	if len(body) == 0 {
		return nil, false
	}
	...
	entry, err := ScoreEntryFromJSON([]byte(valueStr))
	if err != nil {
		return nil, false
	}
...
// incrRedisScore atomically increments a score in Redis
func (ctx *httpContext) incrRedisScore(fingerprint string, increment int) {
	if ctx.config.RedisCluster == "" {
		return
	}

	key := ScoreKey(fingerprint)
	path := fmt.Sprintf("/INCRBY/%s/%s", key, strconv.Itoa(increment))
	...
	if err != nil {
		ctx.logError("failed to dispatch Redis score increment: %v", err)
	}
}
